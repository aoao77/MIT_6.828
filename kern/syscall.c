...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	env->env_tf.tf_cs |= 0x03;
	assert((env->env_tf.tf_cs & 0x03) == 0x03);
	//interrupts enabled
	env->env_tf.tf_eflags |= FL_IF;
	//IOPL of 0
	env->env_tf.tf_eflags ^= FL_IOPL_3;
	assert((env->env_tf.tf_eflags & FL_IOPL_3) == 0);
	//eip & esp
	//Q:supplied us with a good address?
	// assert(env->env_tf.tf_eip % PGSIZE == 0);
	// assert(env->env_tf.tf_esp % PGSIZE == 0);
	return 0;
}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	int r;
	// -E_BAD_ENV
	if((r = envid2env(srcenvid, &src_env, 1) < 0) || (r = envid2env(dstenvid, &dst_env, 1) < 0))
		return r;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if ((size_t)srcva < UTOP && (((perm & ~PTE_SYSCALL) != 0) || ((perm & (PTE_U | PTE_P)) != (PTE_U | PTE_P))))
		return -E_INVAL;

	if ((size_t)srcva < UTOP && ((pp = page_lookup(curenv->env_pgdir, srcva, &pte_item)) == NULL))
		return -E_INVAL;

	if ((perm & PTE_W) && (pte_item != NULL) && !(*pte_item & PTE_W))
		return -E_INVAL;
	
	if(((size_t)dst_env->env_ipc_dstva < UTOP) && (pp != NULL) && (r = page_insert(dst_env->env_pgdir, pp, dst_env->env_ipc_dstva, perm)) < 0)
		return r;
	...
	if ((size_t)dst_env->env_ipc_dstva < UTOP && (size_t)srcva < UTOP) {
